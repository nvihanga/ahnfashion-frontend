...
//         // Step 2: Add to FinishedGoodStock table
//         for (const variant of updatedVariants) {
//           const qty = parseInt(backendSizes[variant.size] || 0);
//           if (qty > 0) {
//             const variantInDb = finishedGood.finishedGoodVariants.find((v) => v.size === variant.size) || {};
//             const stockData = {
//               finishedGoodVariantID: variantInDb.variantID || null, // This might need adjustment based on backend response
//               quantityReceived: qty,
//               receivedDate: entry.date,
//             };

//             const stockResponse = await axios.post("http://localhost:8085/api/stock", stockData);
  ...
  const handlePublish = async () => {
    ...
    try {
      for (const entry of stockList) {
        ...
        const updatedFinishedGoodResponse = await axios.get(
          `http://localhost:8085/api/v1/finishedGood/${finishedGood.finishId}`
        );
